...
Options:
  ...
  --enroll    Run sudo jamf enroll -prompt before any softwareupdate activity.
              Default: disabled.
...
run_jamf_enroll() {
  ...
  log "Running sudo jamf enroll -prompt..."
  sudo "$JAMF_BIN" enroll -prompt || die "sudo jamf enroll -prompt failed."
}
